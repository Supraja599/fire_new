...
class _AlarmPanelInspectionPageState extends State<AlarmPanelInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No alarm panel found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    await LocalDB.saveSingleModuleRecord(
      moduleCode: "alarm_panel",
      recordType: "equipment",
      item: item!,
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Alarm Panel Inspection Saved Locally")));
  }
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  const Icon(Icons.developer_board, color: Colors.red, size: 28),
                ],
              ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Panel Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        child: ListView(
          children: [
                  ...
                  children: [
                    TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: "Enter Panel ID (e.g. SOS-PNL-01)",
            ...
            if (item != null) buildDetails()
              ...
              const Center(child: Padding(padding: EdgeInsets.all(40), child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.search, size: 80, color: Colors.grey), Text("Enter Panel ID to begin", style: TextStyle(color: Colors.grey))]))),
          ],
        ),
      ),
          ...
          children: [
            Expanded(
              child: ElevatedButton.icon(
                ...
                onPressed: () {
            if (item != null) {
              Navigator.push(context, MaterialPageRoute(builder: (_) => AlarmPanelChecklistPage(selectedEquipment: item)));
            } else {
              Navigator.push(context, MaterialPageRoute(builder: (_) => GuidedCaptureWizardPage(
                selectedEquipment: item,
                equipmentImage: 'assets/alarm_panel.png',
                nextScreen: AlarmPanelChecklistPage(selectedEquipment: item),
              )));
            }
          },
              ),
