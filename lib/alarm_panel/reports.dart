...
class _AlarmPanelReportsPageState extends State<AlarmPanelReportsPage> {
  ...
  @override
  void initState() { super.initState(); updateDateFields(); _prefillLatestInspection(); }
  ...
  Future<Map<String, List<Map<String, dynamic>>>> _fetchData() async {
    ...
    List<Map<String, dynamic>> byStatus(String status) {
      if (status == "active") return equipment.where((e) => e["status_bucket"]?.toString() == "active" || e["status_bucket"]?.toString() == "upcoming").toList();
  ...
  Future<void> _prefillLatestInspection() async {
    try {
      final pendingList = await LocalDB.getPendingModuleInspections(moduleCode: AlarmPanelApiService.moduleCode);
            ...
            SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  _actionBtn("Generate PDF Report", Icons.picture_as_pdf, Colors.red, _generatePdf),
                  const SizedBox(height: 15),
                  _actionBtn("Download Excel Sheet", Icons.table_chart, Colors.green, _downloadExcel),
                ],
              ),
            ),
          ],
