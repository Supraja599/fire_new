...
class SCBAUnitsPlantHealthPage extends StatefulWidget {
  const SCBAUnitsPlantHealthPage({super.key});
  @override
  State<SCBAUnitsPlantHealthPage> createState() => _SCBAUnitsPlantHealthPageState();
}

class _SCBAUnitsPlantHealthPageState extends State<SCBAUnitsPlantHealthPage> {
  ...
  void _openStatusList(String s, String t, Color c) {
    ...
    Navigator.push(context, MaterialPageRoute(builder: (_) => _SCBAUnitsStatusListPage(title: t, color: c, icon: Icons.air, items: list, asset: "assets/scba_unit.png")));
  }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          Row(children: [_box("ACTIVE", active, Colors.green, () => _openStatusList("active", "Active SCBA Units", Colors.green)), const SizedBox(width: 12), _box("SERVICE", service, Colors.orange, () => _openStatusList("needs-service", "Needs Service", Colors.orange))]),
          const SizedBox(height: 12),
          Row(children: [_box("INSPECT", inspect, Colors.blue, () => _openStatusList("due-inspection", "Due Inspection", Colors.blue)), const SizedBox(width: 12), _box("EXPIRED", expired, Colors.red, () => _openStatusList("expired", "Expired SCBA Units", Colors.red))]),
...
class _SCBAUnitsStatusListPage extends StatelessWidget {
  final String title; final Color color; final IconData icon; final List<Map<String, dynamic>> items; final String asset;
  const _SCBAUnitsStatusListPage({required this.title, required this.color, required this.icon, required this.items, required this.asset});
  @override
  Widget build(BuildContext context) {
    return Scaffold(
