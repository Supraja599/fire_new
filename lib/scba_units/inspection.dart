...
class SCBAUnitsInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const SCBAUnitsInspectionPage({super.key, this.preScannedId});

  @override
  State<SCBAUnitsInspectionPage> createState() => _SCBAUnitsInspectionPageState();
}

class _SCBAUnitsInspectionPageState extends State<SCBAUnitsInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "scba_unit") {
        setState(() {
          loading = false;
          error = "This is not SCBA Unit equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No SCBA Units found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    await LocalDB.saveSingleModuleRecord(
      moduleCode: "scba_units",
      recordType: "equipment",
      item: item!,
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("SCBA Units Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("SCBA Units Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => SCBAUnitsChecklistPage(selectedEquipment: item)));
                } else {
