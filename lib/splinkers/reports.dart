...
class _SprinklerReportsPageState extends State<SprinklerReportsPage> {
  ...
  Future<Map<String, List<Map<String, dynamic>>>> _fetchData() async {
    ...
    List<Map<String, dynamic>> byStatus(String status) {
      if (status == "active") return equipment.where((item) => item["status_bucket"]?.toString() == "active" || item["status_bucket"]?.toString() == "upcoming").toList();
  ...
  Future<void> _prefillLatestInspection() async {
    try {
      final pendingList = await LocalDB.getPendingModuleInspections(moduleCode: SprinklerApiService.moduleCode);
