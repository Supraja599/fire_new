...
class SprinklerScanPage extends StatefulWidget {
  ...
  @override
  State<SprinklerScanPage> createState() => _SprinklerScanPageState();
}
                  ...
                  children: [
                    TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: "Enter SOS ID (e.g. SOS-SPL-01)",
                    ...
                    if (filteredEquipment.isNotEmpty)
                      ListView.builder(
                        ...
                        itemBuilder: (context, i) {
                          ...
                          return ListTile(
                            dense: true,
                            title: Text((e["sos_code"] ?? e["equipment_id"] ?? "-").toString(), style: const TextStyle(fontWeight: FontWeight.bold)),
                            subtitle: Text(e["location_name"]?.toString() ?? "General Area"),
                            trailing: const Icon(Icons.arrow_forward_ios, size: 14),
                            onTap: () {
                              idController.text = (e["sos_code"] ?? e["equipment_id"]).toString();
          ...
          children: [
            Expanded(
              child: ElevatedButton.icon(
                ...
                onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SprinklerChecklistPage(selectedEquipment: item))),
              ),
            ),
            if (item != null) ...[
              const SizedBox(width: 12),
