...
class SignageInspectionPage extends StatefulWidget {
  ...
  const SignageInspectionPage({super.key, this.preScannedId});

  @override
  State<SignageInspectionPage> createState() => _SignageInspectionPageState();
}
...
class _SignageInspectionPageState extends State<SignageInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "signage") {
        setState(() {
          loading = false;
          error = "This is not Signage equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No Signage found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Signage Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Signage Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            if (item == null && !loading) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFF512DA8), width: 2.5),
                  boxShadow: [BoxShadow(color: const Color(0xFF512DA8).withValues(alpha: 0.08), blurRadius: 15)],
                ),
                child: ClipRRect(
                ...
                child: Column(children: [
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: Icon(Icons.search, color: const Color(0xFF512DA8)),
                      filled: true,
            ...
            if (loading) Center(child: Padding(padding: const EdgeInsets.all(20), child: CircularProgressIndicator(color: const Color(0xFF512DA8)))),
            if (item != null)
              Container(
                ...
                child: Column(children: [
                  Text(
                    ...
                    style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: const Color(0xFF512DA8)),
                  ),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => SignageChecklistPage(selectedEquipment: item)));
                } else {
