...
class SignagePlantHealthPage extends StatefulWidget {
  const SignagePlantHealthPage({super.key});
  @override
  State<SignagePlantHealthPage> createState() => _SignagePlantHealthPageState();
}

class _SignagePlantHealthPageState extends State<SignagePlantHealthPage> {
  ...
  void _openStatusList(String s, String t, Color c) {
    ...
    Navigator.push(context, MaterialPageRoute(builder: (_) => _SignageStatusListPage(title: t, color: c, icon: Icons.medical_services, items: list, asset: "assets/signage.png")));
  }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          Row(children: [_box("ACTIVE", active, Colors.green, () => _openStatusList("active", "Active Signage", Colors.green)), const SizedBox(width: 12), _box("SERVICE", service, Colors.orange, () => _openStatusList("needs-service", "Needs Service", Colors.orange))]),
          const SizedBox(height: 12),
          Row(children: [_box("INSPECT", inspect, Colors.blue, () => _openStatusList("due-inspection", "Due Inspection", Colors.blue)), const SizedBox(width: 12), _box("EXPIRED", expired, Colors.red, () => _openStatusList("expired", "Expired Signage", Colors.red))]),
...
class _SignageStatusListPage extends StatelessWidget {
  final String title; final Color color; final IconData icon; final List<Map<String, dynamic>> items; final String asset;
  const _SignageStatusListPage({required this.title, required this.color, required this.icon, required this.items, required this.asset});
  @override
  Widget build(BuildContext context) {
    return Scaffold(
