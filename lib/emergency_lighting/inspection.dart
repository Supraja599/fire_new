...
class EmergencyLightingInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const EmergencyLightingInspectionPage({super.key, this.preScannedId});

  @override
  State<EmergencyLightingInspectionPage> createState() => _EmergencyLightingInspectionPageState();
}

class _EmergencyLightingInspectionPageState extends State<EmergencyLightingInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "emergency_light") {
        setState(() {
          loading = false;
          error = "This is not Emergency Lighting equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No Emergency Lighting found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    await LocalDB.saveSingleModuleRecord(
      moduleCode: "emergency_lighting",
      recordType: "equipment",
      item: item!,
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Emergency Lighting Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Emergency Lighting Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => EmergencyLightingChecklistPage(selectedEquipment: item)));
                } else {
