...
class EmergencyLightingAlertsPage extends StatefulWidget {
  const EmergencyLightingAlertsPage({super.key});
  @override
  State<EmergencyLightingAlertsPage> createState() => _EmergencyLightingAlertsPageState();
}

class _EmergencyLightingAlertsPageState extends State<EmergencyLightingAlertsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading ? const Center(child: CircularProgressIndicator()) : alerts.isEmpty ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Image.asset("assets/emergency_lighting.png", height: 120, opacity: const AlwaysStoppedAnimation(0.3), errorBuilder: (c,e,s) => Icon(Icons.lightbulb, size: 100, color: Colors.grey.shade300)),
