...
class EmergencyLightingReportsPage extends StatefulWidget {
  const EmergencyLightingReportsPage({super.key});
  @override
  State<EmergencyLightingReportsPage> createState() => _EmergencyLightingReportsPageState();
}

class _EmergencyLightingReportsPageState extends State<EmergencyLightingReportsPage> {
  final api = EmergencyLightingApiService();
  DateTime startDate = DateTime.now().subtract(const Duration(days: 30));
  DateTime endDate = DateTime.now();
  String selectedPlant = "Emergency Lighting";
      ...
      pdf.addPage(pw.MultiPage(maxPages: 1000, 
          ...
          build: (context) => [
            ...
            pw.Row(
              ...
              children: [
                pw.Text("ELTRIVE EMERGENCY LIGHTING SAFETY REPORT", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
                if (logoImage != null)
  ...
  Future<void> generateExcel() async {
    ...
    try {
      ...
      final directory = await getApplicationDocumentsDirectory();
      final path = "${directory.path}/emergency_lighting_Report_${DateTime.now().millisecondsSinceEpoch}.xlsx";
  ...
  Future<void> _prefillLatestInspection() async {
    try {
      final pendingList = await LocalDB.getPendingModuleInspections(moduleCode: EmergencyLightingApiService.moduleCode);
                ...
                children: [
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          DropdownButtonFormField<String>(value: selectedPlant, isExpanded: true, items: ["Emergency Lighting"].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (val) => setState(() => selectedPlant = val!), decoration: const InputDecoration(labelText: "Plant", border: OutlineInputBorder())),
                          const SizedBox(height: 15),
                          DropdownButtonFormField<String>(value: ensureDropdownValue(selectedUnit, ["UNIT-1", "UNIT-2", "UNIT-3"]), isExpanded: true, items: ["UNIT-1", "UNIT-2", "UNIT-3"].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (val) => setState(() => selectedUnit = val!), decoration: const InputDecoration(labelText: "Unit", border: OutlineInputBorder())),
                          const SizedBox(height: 15),
                          Row(children: [
