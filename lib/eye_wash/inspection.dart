...
class EyeWashInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const EyeWashInspectionPage({super.key, this.preScannedId});

  @override
  State<EyeWashInspectionPage> createState() => _EyeWashInspectionPageState();
}

class _EyeWashInspectionPageState extends State<EyeWashInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "eyewash_station") {
        setState(() {
          loading = false;
          error = "This is not Eyewash Station equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No Eye Wash found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Eye Wash Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Eye Wash Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => EyeWashChecklistPage(selectedEquipment: item)));
                } else {
