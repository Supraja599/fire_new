...
class AmbulanceInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const AmbulanceInspectionPage({super.key, this.preScannedId});

  @override
  State<AmbulanceInspectionPage> createState() => _AmbulanceInspectionPageState();
}

class _AmbulanceInspectionPageState extends State<AmbulanceInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "ambulance") {
        setState(() {
          loading = false;
          error = "This is not Ambulance equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No Ambulance found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Ambulance Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Ambulance Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
