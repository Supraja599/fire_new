...
class AmbulanceAlertsPage extends StatefulWidget {
  const AmbulanceAlertsPage({super.key});
  @override
  State<AmbulanceAlertsPage> createState() => _AmbulanceAlertsPageState();
}

class _AmbulanceAlertsPageState extends State<AmbulanceAlertsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading ? const Center(child: CircularProgressIndicator()) : alerts.isEmpty ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Image.asset("assets/ambulance.png", height: 120, opacity: const AlwaysStoppedAnimation(0.3), errorBuilder: (c,e,s) => Icon(Icons.medical_services, size: 100, color: Colors.grey.shade300)),
