...
class AmbulanceReportsPage extends StatefulWidget {
  const AmbulanceReportsPage({super.key});
  @override
  State<AmbulanceReportsPage> createState() => _AmbulanceReportsPageState();
}

class _AmbulanceReportsPageState extends State<AmbulanceReportsPage> {
  final api = AmbulanceApiService();
  DateTime startDate = DateTime.now().subtract(const Duration(days: 30));
  DateTime endDate = DateTime.now();
  String selectedPlant = "Ambulance";
  ...
  Future<void> generatePDF() async {
    ...
    try {
      ...
      pdf.addPage(pw.MultiPage(maxPages: 1000, 
          ...
          build: (context) => [
            ...
            pw.Row(
              ...
              children: [
                pw.Text("ELTRIVE AMBULANCE SAFETY REPORT", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
                if (logoImage != null)
      ...
      final path = "${directory.path}/ambulance_Report_${DateTime.now().millisecondsSinceEpoch}.pdf";
  ...
  Future<void> generateExcel() async {
    ...
    try {
      ...
      final directory = await getApplicationDocumentsDirectory();
      final path = "${directory.path}/ambulance_Report_${DateTime.now().millisecondsSinceEpoch}.xlsx";
  ...
  Future<void> _prefillLatestInspection() async {
    try {
      final pendingList = await LocalDB.getPendingModuleInspections(moduleCode: AmbulanceApiService.moduleCode);
                ...
                children: [
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          DropdownButtonFormField<String>(value: selectedPlant, isExpanded: true, items: ["Ambulance"].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (val) => setState(() => selectedPlant = val!), decoration: const InputDecoration(labelText: "Plant", border: OutlineInputBorder())),
                          const SizedBox(height: 15),
