...
class EmergencyCommAlertsPage extends StatefulWidget {
  const EmergencyCommAlertsPage({super.key});
  @override
  State<EmergencyCommAlertsPage> createState() => _EmergencyCommAlertsPageState();
}

class _EmergencyCommAlertsPageState extends State<EmergencyCommAlertsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading ? const Center(child: CircularProgressIndicator()) : alerts.isEmpty ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Image.asset("assets/emergency_comm.png", height: 120, opacity: const AlwaysStoppedAnimation(0.3), errorBuilder: (c,e,s) => Icon(Icons.medical_services, size: 100, color: Colors.grey.shade300)),
