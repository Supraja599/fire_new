...
class EmergencyCommInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const EmergencyCommInspectionPage({super.key, this.preScannedId});

  @override
  State<EmergencyCommInspectionPage> createState() => _EmergencyCommInspectionPageState();
}

class _EmergencyCommInspectionPageState extends State<EmergencyCommInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "emergency_comm") {
        setState(() {
          loading = false;
          error = "This is not Emergency Comm equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No Emergency Comm found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Emergency Comm Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Emergency Comm Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            if (item == null && !loading) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFF303F9F), width: 2.5),
                  boxShadow: [BoxShadow(color: const Color(0xFF303F9F).withValues(alpha: 0.08), blurRadius: 15)],
                ),
                child: ClipRRect(
                ...
                child: Column(children: [
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: Icon(Icons.search, color: const Color(0xFF303F9F)),
                      filled: true,
            ...
            if (loading) Center(child: Padding(padding: const EdgeInsets.all(20), child: CircularProgressIndicator(color: const Color(0xFF303F9F)))),
            if (item != null)
              Container(
                ...
                child: Column(children: [
                  Text(
                    ...
                    style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: const Color(0xFF303F9F)),
                  ),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => EmergencyCommChecklistPage(selectedEquipment: item)));
                } else {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => GuidedCaptureWizardPage(
                    selectedEquipment: item,
                    equipmentImage: 'assets/emergency_comm.webp',
                    nextScreen: EmergencyCommChecklistPage(selectedEquipment: item),
                  )));
                }
              },
            ),
