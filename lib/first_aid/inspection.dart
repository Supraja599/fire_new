...
class FirstAidInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const FirstAidInspectionPage({super.key, this.preScannedId});

  @override
  State<FirstAidInspectionPage> createState() => _FirstAidInspectionPageState();
}

class _FirstAidInspectionPageState extends State<FirstAidInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "first_aid_kit") {
        setState(() {
          loading = false;
          error = "This is not First Aid Kit equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No First Aid found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    await LocalDB.saveSingleModuleRecord(
      moduleCode: "first_aid",
      recordType: "equipment",
      item: item!,
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("First Aid Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("First Aid Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            if (item == null && !loading) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [BoxShadow(color: const Color(0xFF2E7D32).withValues(alpha: 0.08), blurRadius: 15)],
                ),
                child: ClipRRect(
                ...
                child: Column(children: [
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: Icon(Icons.search, color: const Color(0xFF2E7D32)),
                      filled: true,
            ...
            if (loading) Center(child: Padding(padding: const EdgeInsets.all(20), child: CircularProgressIndicator(color: const Color(0xFF2E7D32)))),
            if (item != null)
              Container(
                ...
                child: Column(children: [
                  Text(
                    ...
                    style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: const Color(0xFF2E7D32)),
                  ),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => FirstAidChecklistPage(selectedEquipment: item)));
                } else {
