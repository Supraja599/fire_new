...
class _CO2SystemPlantHealthPageState extends State<CO2SystemPlantHealthPage> {
  ...
  void _openStatusList(String s, String t, Color c) {
    final list = equipment
        .where((item) => (item["status_bucket"]?.toString() ?? item["status"]?.toString() ?? "").toLowerCase().contains(s.toLowerCase()))
        .toList();
    Navigator.push(
      context,
      MaterialPageRoute(
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          Row(children: [_box("ACTIVE", active, Colors.green, 'assets/co2_system.png', () => _openStatusList("active", "Active CO2System", Colors.green)), const SizedBox(width: 12), _box("SERVICE", service, Colors.orange, 'assets/chemical_shower.png', () => _openStatusList("needs-service", "Needs Service", Colors.orange))]),
  ...
  Widget _box(String t, int v, Color c, String img, VoidCallback onTap) => Expanded(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text("$v", style: const TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.w900)),
          ],
        ),
      ),
    ),
