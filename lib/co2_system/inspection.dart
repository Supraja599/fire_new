...
class CO2SystemInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const CO2SystemInspectionPage({super.key, this.preScannedId});

  @override
  State<CO2SystemInspectionPage> createState() => _CO2SystemInspectionPageState();
}

class _CO2SystemInspectionPageState extends State<CO2SystemInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "suppression_system") {
        setState(() {
          loading = false;
          error = "This is not CO2 System equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No CO2 System found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    await LocalDB.saveSingleModuleRecord(
      moduleCode: "co2_system",
      recordType: "equipment",
      item: item!,
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("CO2 System Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("CO2 System Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            if (item == null && !loading) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFF00838F), width: 2.5),
                  boxShadow: [BoxShadow(color: const Color(0xFF00838F).withValues(alpha: 0.08), blurRadius: 15)],
                ),
                child: ClipRRect(
                ...
                child: Column(children: [
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: Icon(Icons.search, color: const Color(0xFF00838F)),
                      filled: true,
            ...
            if (loading) Center(child: Padding(padding: const EdgeInsets.all(20), child: CircularProgressIndicator(color: const Color(0xFF00838F)))),
            if (item != null)
              Container(
                ...
                child: Column(children: [
                  Text(
                    ...
                    style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: const Color(0xFF00838F)),
                  ),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => CO2SystemChecklistPage(selectedEquipment: item)));
                } else {
