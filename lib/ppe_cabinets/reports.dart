...
class PPECabinetsReportsPage extends StatefulWidget {
  const PPECabinetsReportsPage({super.key});
  @override
  State<PPECabinetsReportsPage> createState() => _PPECabinetsReportsPageState();
}

class _PPECabinetsReportsPageState extends State<PPECabinetsReportsPage> {
  final api = PPECabinetsApiService();
  DateTime startDate = DateTime.now().subtract(const Duration(days: 30));
  DateTime endDate = DateTime.now();
  String selectedPlant = "PPECabinets";
  ...
  Future<void> generatePDF() async {
    ...
    try {
      ...
      pdf.addPage(pw.MultiPage(maxPages: 1000, 
          ...
          build: (context) => [
            ...
            pw.Row(
              ...
              children: [
                pw.Text("ELTRIVE PPE CABINETS SAFETY REPORT", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
                if (logoImage != null)
      ...
      final path = "${directory.path}/ppe_cabinets_Report_${DateTime.now().millisecondsSinceEpoch}.pdf";
  ...
  Future<void> generateExcel() async {
    ...
    try {
      ...
      final directory = await getApplicationDocumentsDirectory();
      final path = "${directory.path}/ppe_cabinets_Report_${DateTime.now().millisecondsSinceEpoch}.xlsx";
  ...
  Future<void> _prefillLatestInspection() async {
    try {
      final pendingList = await LocalDB.getPendingModuleInspections(moduleCode: PPECabinetsApiService.moduleCode);
                ...
                children: [
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          DropdownButtonFormField<String>(value: selectedPlant, isExpanded: true, items: ["PPECabinets"].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (val) => setState(() => selectedPlant = val!), decoration: const InputDecoration(labelText: "Plant", border: OutlineInputBorder())),
                          const SizedBox(height: 15),
