...
class PPECabinetsScanPage extends StatefulWidget {
  const PPECabinetsScanPage({super.key});
  @override
  State<PPECabinetsScanPage> createState() => _PPECabinetsScanPageState();
}

class _PPECabinetsScanPageState extends State<PPECabinetsScanPage> {
    ...
    showDialog(context: context, builder: (c) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.red), onPressed: () async {
          ...
          await LocalDB.saveSingleModuleRecord(moduleCode: PPECabinetsApiService.moduleCode, recordType: "equipment", item: item!);
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(child: Column(children: [
        ...
        if (item != null) Container(margin: const EdgeInsets.symmetric(horizontal: 15), padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20)), child: Column(children: [
          ...
          SizedBox(width: double.infinity, child: ElevatedButton.icon(style: ElevatedButton.styleFrom(backgroundColor: Colors.red, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))), icon: const Icon(Icons.list_alt, color: Colors.white), label: const Text("OPEN CHECKLIST", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const PPECabinetsChecklistPage())))),
        ])),
