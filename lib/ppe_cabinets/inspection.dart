...
class PPECabinetsInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const PPECabinetsInspectionPage({super.key, this.preScannedId});

  @override
  State<PPECabinetsInspectionPage> createState() => _PPECabinetsInspectionPageState();
}

class _PPECabinetsInspectionPageState extends State<PPECabinetsInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "ppe_station") {
        setState(() {
          loading = false;
          error = "This is not PPE Cabinet equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No PPE Cabinets found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    await LocalDB.saveSingleModuleRecord(
      moduleCode: "ppe_cabinets",
      recordType: "equipment",
      item: item!,
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("PPE Cabinets Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("PPE Cabinets Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => PPECabinetsChecklistPage(selectedEquipment: item)));
                } else {
