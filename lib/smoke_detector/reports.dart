  ...
  Future<void> generatePDF() async {
    ...
    try {
      ...
      pdf.addPage(pw.MultiPage(
        build: (context) => [
          pw.Header(level: 0, child: pw.Text("SMOKE DETECTOR SAFETY REPORT", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold))),
          pw.SizedBox(height: 10),
          pw.Text("Plant: $selectedPlant | Unit: $selectedUnit"),
          pw.Text("Period: ${startController.text} to ${endController.text}"),
          pw.SizedBox(height: 20),
          pw.Table.fromTextArray(
            headers: ["SOS Code", "Location", "Status", "Last Inspection"],
            data: allData.map((e) => [
              (e['sos_code'] ?? e['id'] ?? '-').toString(),
              (e['location_name'] ?? e['building_name'] ?? '-').toString(),
              (e['status_label'] ?? '-').toString(),
              (e['last_inspection_date'] ?? e['last_service_date'] ?? '-').toString(),
            ]).toList(),
          ),
        ],
      ));
  ...
  Future<void> generateExcel() async {
    ...
    try {
      ...
      dataMap.forEach((status, list) {
        ...
        sheet.appendRow(["SOS CODE", "LOCATION", "STATUS", "LAST INSPECTION"]);
        for (var item in list) {
          sheet.appendRow([
            item['sos_code'] ?? item['id'] ?? '-',
            item['location_name'] ?? item['building_name'] ?? '-',
            status,
            item['last_inspection_date'] ?? item['last_service_date'] ?? '-',
          ]);
        }
      });
