...
class WindSockInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const WindSockInspectionPage({super.key, this.preScannedId});

  @override
  State<WindSockInspectionPage> createState() => _WindSockInspectionPageState();
}

class _WindSockInspectionPageState extends State<WindSockInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "wind_sock") {
        setState(() {
          loading = false;
          error = "This is not Wind Sock equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No Wind Sock found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Wind Sock Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Wind Sock Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
