...
class ChemicalShowerReportsPage extends StatefulWidget {
  const ChemicalShowerReportsPage({super.key});
  @override
  State<ChemicalShowerReportsPage> createState() => _ChemicalShowerReportsPageState();
}

class _ChemicalShowerReportsPageState extends State<ChemicalShowerReportsPage> {
  final api = ChemicalShowerApiService();
  DateTime startDate = DateTime.now().subtract(const Duration(days: 30));
  DateTime endDate = DateTime.now();
  String selectedPlant = "ChemicalShower";
  ...
  Future<void> generatePDF() async {
    ...
    try {
      ...
      pdf.addPage(pw.MultiPage(build: (context) => [
        pw.Header(level: 0, child: pw.Text("Chemical Shower SAFETY REPORT", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold))),
      ...
      final path = "${directory.path}/chemical_shower_Report_${DateTime.now().millisecondsSinceEpoch}.pdf";
  ...
  Future<void> generateExcel() async {
    ...
    try {
      ...
      final directory = await getApplicationDocumentsDirectory();
      final path = "${directory.path}/chemical_shower_Report_${DateTime.now().millisecondsSinceEpoch}.xlsx";
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(children: [
        Card(elevation: 8, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), child: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
          DropdownButtonFormField<String>(value: selectedPlant, isExpanded: true, items: ["ChemicalShower"].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (val) => setState(() => selectedPlant = val!), decoration: const InputDecoration(labelText: "Plant", border: OutlineInputBorder())),
