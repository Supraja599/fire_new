...
class SandBucketInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const SandBucketInspectionPage({super.key, this.preScannedId});

  @override
  State<SandBucketInspectionPage> createState() => _SandBucketInspectionPageState();
}

class _SandBucketInspectionPageState extends State<SandBucketInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "sand_bucket") {
        setState(() {
          loading = false;
          error = "This is not Sand Bucket equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No Sand Bucket found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Sand Bucket Inspection Saved Locally")));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Sand Bucket Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => SandBucketChecklistPage(selectedEquipment: item)));
                } else {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => GuidedCaptureWizardPage(
                    selectedEquipment: item,
                    equipmentImage: 'assets/sand_bucket.png',
                    nextScreen: SandBucketChecklistPage(selectedEquipment: item),
                  )));
                }
              },
            ),
