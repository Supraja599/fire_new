...
class FireDoorReportsPage extends StatefulWidget {
  const FireDoorReportsPage({super.key});
  @override
  State<FireDoorReportsPage> createState() => _FireDoorReportsPageState();
}

class _FireDoorReportsPageState extends State<FireDoorReportsPage> {
  ...
  T? ensureDropdownValue<T>(T? value, List<T> items) {
  ...
  Future<void> generatePDF() async {
    ...
    try {
      ...
      pdf.addPage(pw.MultiPage(maxPages: 1000, 
          ...
          build: (context) => [
            ...
            pw.Row(
              ...
              children: [
                pw.Text("ELTRIVE FIRE DOOR SAFETY REPORT", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
                if (logoImage != null)
      ...
      final path = "${directory.path}/fire_door_Report_${DateTime.now().millisecondsSinceEpoch}.pdf";
  ...
  Future<void> generateExcel() async {
    ...
    try {
      ...
      final directory = await getApplicationDocumentsDirectory();
      final path = "${directory.path}/fire_door_Report_${DateTime.now().millisecondsSinceEpoch}.xlsx";
  ...
  Future<void> _prefillLatestInspection() async {
    try {
      final pendingList = await LocalDB.getPendingModuleInspections(moduleCode: FireDoorApiService.moduleCode);
  ...
  value: ensureDropdownValue<String>(selectedPlant, ["Fire Door"]),
  isExpanded: true,
  items: ["Fire Door"].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
  onChanged: (val) => setState(() => selectedPlant = val),
  decoration: const InputDecoration(labelText: "Plant", border: OutlineInputBorder()),
),
                          const SizedBox(height: 15),
                          DropdownButtonFormField<String>(
  value: ensureDropdownValue<String>(selectedUnit, ["UNIT-1", "UNIT-2", "UNIT-3"]),
  isExpanded: true,
