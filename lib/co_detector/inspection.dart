...
class CODetectorInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const CODetectorInspectionPage({super.key, this.preScannedId});

  @override
  State<CODetectorInspectionPage> createState() => _CODetectorInspectionPageState();
}

class _CODetectorInspectionPageState extends State<CODetectorInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "co_detector") {
        setState(() {
          loading = false;
          error = "This is not CO Detector equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No CO Detector found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("CO Detector Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("CO Detector Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => CODetectorChecklistPage(selectedEquipment: item)));
                } else {
