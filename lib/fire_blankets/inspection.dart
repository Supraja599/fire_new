...
class FireBlanketsInspectionPage extends StatefulWidget {
  ...
  const FireBlanketsInspectionPage({super.key, this.preScannedId});

  @override
  State<FireBlanketsInspectionPage> createState() => _FireBlanketsInspectionPageState();
}

class _FireBlanketsInspectionPageState extends State<FireBlanketsInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "fire_blanket") {
        setState(() {
          loading = false;
          error = "This is not Fire Blanket equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No Fire Blankets found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    await LocalDB.saveSingleModuleRecord(
      moduleCode: "fire_blankets",
      recordType: "equipment",
      item: item!,
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Fire Blankets Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Fire Blankets Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            if (item == null && !loading) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFFD84315), width: 2.5),
                  boxShadow: [BoxShadow(color: const Color(0xFFD84315).withValues(alpha: 0.08), blurRadius: 15)],
                ),
                child: ClipRRect(
                ...
                child: Column(children: [
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: Icon(Icons.search, color: const Color(0xFFD84315)),
                      filled: true,
            ...
            if (loading) Center(child: Padding(padding: const EdgeInsets.all(20), child: CircularProgressIndicator(color: const Color(0xFFD84315)))),
            if (item != null)
              Container(
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => FireBlanketsChecklistPage(selectedEquipment: item)));
                } else {
