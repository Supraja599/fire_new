...
class FireBlanketsReportsPage extends StatefulWidget {
  const FireBlanketsReportsPage({super.key});
  @override
  State<FireBlanketsReportsPage> createState() => _FireBlanketsReportsPageState();
}

class _FireBlanketsReportsPageState extends State<FireBlanketsReportsPage> {
  final api = FireBlanketsApiService();
  DateTime startDate = DateTime.now().subtract(const Duration(days: 30));
  DateTime endDate = DateTime.now();
  String selectedPlant = "FireBlankets";
  ...
  Future<void> generatePDF() async {
    ...
    try {
      ...
      pdf.addPage(pw.MultiPage(maxPages: 1000, 
          ...
          build: (context) => [
            ...
            pw.Row(
              ...
              children: [
                pw.Text("ELTRIVE FIRE BLANKETS SAFETY REPORT", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
                if (logoImage != null)
      ...
      final path = "${directory.path}/fire_blankets_Report_${DateTime.now().millisecondsSinceEpoch}.pdf";
  ...
  Future<void> generateExcel() async {
    ...
    try {
      ...
      final directory = await getApplicationDocumentsDirectory();
      final path = "${directory.path}/fire_blankets_Report_${DateTime.now().millisecondsSinceEpoch}.xlsx";
  ...
  Future<void> _prefillLatestInspection() async {
    try {
      final pendingList = await LocalDB.getPendingModuleInspections(moduleCode: FireBlanketsApiService.moduleCode);
                ...
                children: [
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          DropdownButtonFormField<String>(value: selectedPlant, isExpanded: true, items: ["FireBlankets"].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (val) => setState(() => selectedPlant = val!), decoration: const InputDecoration(labelText: "Plant", border: OutlineInputBorder())),
                          const SizedBox(height: 15),
