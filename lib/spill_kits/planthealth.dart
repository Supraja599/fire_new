...
class SpillKitsPlantHealthPage extends StatefulWidget {
  const SpillKitsPlantHealthPage({super.key});
  @override
  State<SpillKitsPlantHealthPage> createState() => _SpillKitsPlantHealthPageState();
}

class _SpillKitsPlantHealthPageState extends State<SpillKitsPlantHealthPage> {
  ...
  void _openStatusList(String s, String t, Color c) {
    ...
    Navigator.push(context, MaterialPageRoute(builder: (_) => _SpillKitsStatusListPage(title: t, color: c, icon: Icons.medical_services, items: list, asset: "assets/spill_kits.png")));
  }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          Row(children: [_box("ACTIVE", active, Colors.green, () => _openStatusList("active", "Active SpillKits", Colors.green)), const SizedBox(width: 12), _box("SERVICE", service, Colors.orange, () => _openStatusList("needs-service", "Needs Service", Colors.orange))]),
          const SizedBox(height: 12),
          Row(children: [_box("INSPECT", inspect, Colors.blue, () => _openStatusList("due-inspection", "Due Inspection", Colors.blue)), const SizedBox(width: 12), _box("EXPIRED", expired, Colors.red, () => _openStatusList("expired", "Expired SpillKits", Colors.red))]),
...
class _SpillKitsStatusListPage extends StatelessWidget {
  final String title; final Color color; final IconData icon; final List<Map<String, dynamic>> items; final String asset;
  const _SpillKitsStatusListPage({required this.title, required this.color, required this.icon, required this.items, required this.asset});
  @override
  Widget build(BuildContext context) {
    return Scaffold(
