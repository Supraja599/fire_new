...
class SpillKitsInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const SpillKitsInspectionPage({super.key, this.preScannedId});

  @override
  State<SpillKitsInspectionPage> createState() => _SpillKitsInspectionPageState();
}

class _SpillKitsInspectionPageState extends State<SpillKitsInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "spill_kit") {
        setState(() {
          loading = false;
          error = "This is not Spill Kit equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No Spill Kits found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    await LocalDB.saveSingleModuleRecord(
      moduleCode: "spill_kits",
      recordType: "equipment",
      item: item!,
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Spill Kits Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Spill Kits Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            if (item == null && !loading) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFF33691E), width: 2.5),
                  boxShadow: [BoxShadow(color: const Color(0xFF33691E).withValues(alpha: 0.08), blurRadius: 15)],
                ),
                child: ClipRRect(
                ...
                child: Column(children: [
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: Icon(Icons.search, color: const Color(0xFF33691E)),
                      filled: true,
            ...
            if (loading) Center(child: Padding(padding: const EdgeInsets.all(20), child: CircularProgressIndicator(color: const Color(0xFF33691E)))),
            if (item != null)
              Container(
                ...
                child: Column(children: [
                  Text(
                    ...
                    style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: const Color(0xFF33691E)),
                  ),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => SpillKitsChecklistPage(selectedEquipment: item)));
                } else {
