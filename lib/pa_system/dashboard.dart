...
class PASystemDashboard extends StatefulWidget {
  const PASystemDashboard({super.key});
  @override
  State<PASystemDashboard> createState() => _PASystemDashboardState();
}

class _PASystemDashboardState extends State<PASystemDashboard> {
      ...
      body: SafeArea(
        child: Column(children: [
          Padding(
            ...
            child: Row(children: [
              ...
              const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text("PA System", style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: primary)),
                Text("Voice Response Monitor", style: TextStyle(fontSize: 9, color: Colors.grey, fontWeight: FontWeight.bold, letterSpacing: 1.1)),
              ]),
              const Spacer(),
          ...
          Container(
            ...
            child: Row(children: [
              const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text("LIVE STATUS", style: TextStyle(color: Colors.white70, fontSize: 9, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
                SizedBox(height: 5),
                Text("Broadcast Efficiency", style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w900, height: 1.1)),
                SizedBox(height: 5),
                Text("Real-time monitoring of all units.", style: TextStyle(color: Colors.white60, fontSize: 11)),
              ])),
              Image.asset("assets/pa_system.png", height: 60, fit: BoxFit.contain, errorBuilder: (c,e,s) => const Icon(Icons.volume_up, size: 50, color: Colors.white)),
            ]),
          ),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 8),
            child: Row(children: [
              _metricTile("Active Modules", isLoading ? "..." : "$active", Colors.green.shade700),
              const SizedBox(width: 12),
              _metricTile("Health Score", isLoading ? "..." : "$health%", primary),
            ]),
          ),
          Expanded(child: GridView.count(
            ...
            children: [
              _ActionCard("Plant Health", Icons.analytics_outlined, Colors.green, const PASystemPlantHealthPage(), "Device Health"),
              _ActionCard("Safety Alerts", Icons.notification_important_outlined, Colors.red, const PASystemAlertsPage(), "Active Alerts"),
              _ActionCard("Maintenance", Icons.build_circle_outlined, Colors.orange, const PASystemMaintenancePage(), "Services"),
              _ActionCard("Reports", Icons.summarize_outlined, Colors.purple, const PASystemReportsPage(), "History"),
              _ActionCard("Checklist", Icons.fact_check_outlined, Colors.teal, const PASystemChecklistPage(), "Forms"),
              _ActionCard("Inspection", Icons.qr_code_scanner, Colors.blue, const PASystemScanPage(), "Search"),
            ],
          )),
        ]),
      ),
