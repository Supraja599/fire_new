...
class PASystemInspectionPage extends StatefulWidget {
  ...
  const PASystemInspectionPage({super.key, this.preScannedId});

  @override
  State<PASystemInspectionPage> createState() => _PASystemInspectionPageState();
}
...
class _PASystemInspectionPageState extends State<PASystemInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "pa_system") {
        setState(() {
          loading = false;
          error = "This is not PA System equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No PA System found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("PA System Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("PA System Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => PASystemChecklistPage(selectedEquipment: item)));
                } else {
