...
class HoseReelInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const HoseReelInspectionPage({super.key, this.preScannedId});

  @override
  State<HoseReelInspectionPage> createState() => _HoseReelInspectionPageState();
}

class _HoseReelInspectionPageState extends State<HoseReelInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "hose_reel") {
        setState(() {
          loading = false;
          error = "This is not Hose Reel equipment";
          showSearch = true;
        });
        return;
      }
        ...
        child: Row(children: [
          Expanded(
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (item != null) {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => HoseReelChecklistPage(selectedEquipment: item)));
                } else {
