...
class EmergencyExitsReportsPage extends StatefulWidget {
  const EmergencyExitsReportsPage({super.key});
  @override
  State<EmergencyExitsReportsPage> createState() => _EmergencyExitsReportsPageState();
}

class _EmergencyExitsReportsPageState extends State<EmergencyExitsReportsPage> {
  final api = EmergencyExitsApiService();
  DateTime startDate = DateTime.now().subtract(const Duration(days: 30));
  DateTime endDate = DateTime.now();
  String selectedPlant = "Emergency Exits";
  ...
  void initState() {
    super.initState();
    _updateDateFields(); _prefillLatestInspection();
  }
  ...
  Future<void> generatePDF() async {
    ...
    try {
      ...
      if (allData.isEmpty) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("No equipment data found for report")));
      ...
      pdf.addPage(pw.MultiPage(maxPages: 1000, 
          ...
          build: (context) => [
            ...
            pw.Row(
              ...
              children: [
                pw.Text("ELTRIVE EMERGENCY EXIT SAFETY REPORT", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
                if (logoImage != null)
      ...
      final path = "${directory.path}/EmergencyExits_Report_${DateTime.now().millisecondsSinceEpoch}.pdf";
  ...
  Future<void> generateExcel() async {
    ...
    try {
      ...
      final directory = await getApplicationDocumentsDirectory();
      final path = "${directory.path}/EmergencyExits_Report_${DateTime.now().millisecondsSinceEpoch}.xlsx";
      final file = File(path);
  ...
  Future<void> _prefillLatestInspection() async {
    try {
      final pendingList = await LocalDB.getPendingModuleInspections(moduleCode: EmergencyExitsApiService.moduleCode);
                ...
                children: [
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          DropdownButtonFormField<String>(value: selectedPlant, isExpanded: true, items: ["Emergency Exits"].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (val) => setState(() => selectedPlant = val!), decoration: const InputDecoration(labelText: "Plant", border: OutlineInputBorder())),
                          const SizedBox(height: 15),
