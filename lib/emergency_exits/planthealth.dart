...
class EmergencyExitsPlantHealthPage extends StatefulWidget {
  const EmergencyExitsPlantHealthPage({super.key});
  @override
  State<EmergencyExitsPlantHealthPage> createState() => _EmergencyExitsPlantHealthPageState();
}

class _EmergencyExitsPlantHealthPageState extends State<EmergencyExitsPlantHealthPage> {
  ...
  void _openStatusList(String s, String t, Color c) {
    final list = equipment.where((item) => item["status_bucket"]?.toString() == s).toList();
    Navigator.push(context, MaterialPageRoute(builder: (_) => _EmergencyExitsStatusListPage(title: t, color: c, icon: Icons.door_front_door, items: list, asset: "assets/emergency_exit.png")));
  }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          Row(children: [_box("ACTIVE", active, Colors.green, () => _openStatusList("active", "Active Exits", Colors.green)), const SizedBox(width: 12), _box("SERVICE", service, Colors.orange, () => _openStatusList("needs-service", "Needs Service", Colors.orange))]),
          const SizedBox(height: 12),
          Row(children: [_box("INSPECT", inspect, Colors.blue, () => _openStatusList("due-inspection", "Due Inspection", Colors.blue)), const SizedBox(width: 12), _box("EXPIRED", expired, Colors.red, () => _openStatusList("expired", "Expired Exits", Colors.red))]),
...
class _EmergencyExitsStatusListPage extends StatelessWidget {
  final String title; final Color color; final IconData icon; final List<Map<String, dynamic>> items; final String asset;
  const _EmergencyExitsStatusListPage({required this.title, required this.color, required this.icon, required this.items, required this.asset});
  @override
  Widget build(BuildContext context) {
    return Scaffold(
