...
class EmergencyExitsInspectionPage extends StatefulWidget {
  final String? preScannedId;
  const EmergencyExitsInspectionPage({super.key, this.preScannedId});

  @override
  State<EmergencyExitsInspectionPage> createState() => _EmergencyExitsInspectionPageState();
}

class _EmergencyExitsInspectionPageState extends State<EmergencyExitsInspectionPage> {
  ...
  Future<void> fetchDetails(String input) async {
    ...
    try {
      ...
      if (foundModule != null && foundModule != "exit_sign") {
        setState(() {
          loading = false;
          error = "This is not Emergency Exit equipment";
          showSearch = true;
        });
        return;
      }
    ...
    try {
      ...
      if (data == null) {
        setState(() {
          loading = false;
          error = "No Emergency Exits found for this ID";
          showSearch = true;
        });
        return;
      }
  ...
  Future<void> _submitInspection() async {
    ...
    await LocalDB.saveSingleModuleRecord(
      moduleCode: "emergency_exits",
      recordType: "equipment",
      item: item!,
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Emergency Exits Inspection Saved Locally")));
  }
...
Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Emergency Exits Inspection", style: TextStyle(fontWeight: FontWeight.bold)),
